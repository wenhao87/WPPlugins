		...
		if ( isset( $_POST['importlinks'] ) ) {
			...
			foreach (
				array(
					...
					'searchfieldtext', 'catfilterlabel', 'searchnoresultstext', 'addlinkdefaultcat'
				) as $option_name
			) {
				if ( isset( $_POST[$option_name] ) ) {
			...
			foreach (
				array(
					...
					'cat_letter_filter_autoselect', 'cat_letter_filter_showalloption', 'emailsubmitter', 'addlinkakismet'
				)
				as $option_name
			) {
				if ( isset( $_POST[$option_name] ) ) {
